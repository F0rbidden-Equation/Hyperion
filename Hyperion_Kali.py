...
c11 = (colored("[11]", 'blue'))
c11_a = (colored(" Research Google Dork", 'green'))
c12 = (colored("[12]", 'blue'))
c12_a = (colored(" Research Emails ", 'green'))
c13 = (colored("[13]", 'blue'))
c13_a = (colored(" Detected Type DataBase", 'green'))
c14 = (colored("[14]", 'blue'))
c14_a = (colored(" DNS brute Force", 'green'))
c15 = (colored("[15]", 'blue'))
c15_a = (colored(" Profiler Password", 'green'))
...
def Menu_Infos_Network():
    ...
    if verifdns == 0:
        ...
        ipcolor = (colored("[*]|-----> Ip : ", 'green'))
        print(ipcolor, ip)
        dnscolor =   (colored("[*]|-----> Dns : ", 'green'))
        print(dnscolor, dns)
        ...
        # print(w)
        emails = (w.emails)
        emcolor = (colored("[*]|-----> Emails :", 'green'))
        print(emcolor, emails)
        dnsseccolor = (colored("[*]|-----> Dns Secondary :", 'green'))
        dnssec = (w.dnssec)
        print(dnssec)
        namecolor = (colored("[*]|-----> Name :", 'green'))
        name = (w.name)
        print(namecolor, name)
        orgcolor = (colored("[*]|-----> Corporations :", 'green'))
        org = (w.org)
        print(orgcolor, org)
        adresscolor = (colored("[*]|-----> Adress :", 'green'))
        adress = (w.adress)
        print(adresscolor, adress)
        citycolor = (colored("[*]|-----> City :", 'green'))
        city = (w.city)
        print(citycolor, city)
        statecolor = (colored("[*]|-----> State :", 'green'))
        state = (w.state)
        print(statecolor, state)
        zipcolor = (colored("[*]|-----> Zip :", 'green'))
        zipcode = (w.zipcode)
        print(zipcolor, zipcode)
        colorcount = (colored("[*]|-----> Country :", 'green'))
        country2 = (w.country)
        #print(colorcount, country)
        servcolor = (colored("[*]|-----> Names Servers :", 'green'))
        names_servers = (w.name_servers)
        print(servcolor, names_servers)
        creatio_datecolor = (colored("[*]|-----> Creation Date Servers :", 'green'))
        creation_date = (w.creation_date)
        print(creatio_datecolor, creation_date)
        updated_datecolor = (colored("[*]|-----> Last Update Date Servers :", 'green'))
        updated_date = (w.updated_date)
        print(updated_datecolor, updated_date)
        expiration_color = (colored("[*]|-----> Expirations Date Servers :", 'green'))
...
def Menu_Anonymat_Servers():
    ...
    def vpnIP():
        ...
        if vpn == 0:  # False = 0  / True = 1
            print(colored("Security [vpn] =  Actived", 'green'))
        elif vpn == 1:
            print(colored("Security [vpn] = No Actived", 'red'))
        else:
            print(colored("IP adress No Valide", 'red'))
    ...
    def hostingIP():
        ...
        if hosting == 0:  # False = 0  / True = 1
            print(colored("Security [hosting_provider] = Actived", 'green'))
        elif hosting == 1:
    ...
    def TorIP():
        ...
        if Tor == 0:  # False = 0  / True = 1
            print(colored("Security [tor_exit_node] =  Actived", 'green'))
        elif Tor == 1:
            print(colored("Security [tor_exit_node] = No Actived", 'red'))
        else:
            print(colored("IP Adress No Valide", 'red'))
        print("")
...
def Menu_Pages_php():
    ...
    url = input(colored("Enter your Url website (www.example.com/index.php?id=1): ", 'red' ))
...
def Menu_bruteforce_R():
    ...
    if verifdns == 0:
        ip = (socket.gethostbyname(dns))
        hosts = [ip]
        ports = [21, 22, 23, 110, 119, 143, 445, 465, 512, 513, 514, 563, 993, 1433, 2525, 3306, 5432]
        color1 = (colored("|---> [+] Connecting to [", 'green'))
        colorP = (colored("] | port: ", 'green'))
        print(colored("--" * 60, 'green'))
...
def Menu_EmailR():
    menu_retour()
    print(colored("Please patient *multiple research* 1 minute !! "))
    mail_name = input(colored("Enter your name of research (example : steven.gmail) :", 'magenta'))
    print(colored("Please patient *multiples researchs* > *30 secondes* !!!!" , 'red'))
    search_name = subprocess.run("theHarvester -d " + mail_name + " -l 500 -b google", shell=True)
...
def Dns_Force():
    ...
    if verifdns == 0:
        ip = (socket.gethostbyname(dns))
        print(colored(
            "-------please patient !-2 minute --------- [DNS Brute Force] ---------------------------------------",
            'blue'))
...
def Menu_Research_dork():
    menu_retour()
    print("--------------------------------------Dork-Research-Google-advanced------------------------------------------------")
    dork = input(colored("Enter your Dork : ", 'magenta'))  # part research
    for url in search(dork, tld='com', lang='en', num=10, start=0, stop=None, pause=2.0):
...
def menu_retour():
    retour = input(colored("Press 'R' for Return / Press 'Enter' Continue ...", 'red'))
    if retour == ("R"):
        Presentation()
        Select_Mod()
    else:
...
def Presentation():
    ...
    print("")
    print(colored("[Hyperion Program for security testing]" ,"blue"))
    print(colored("*I am not responsible if this tools is diverted and used without authorization*","red"))
    print("")
    ...
    print(c4 + c4_a, " |", c8 + c8_a, " | ", c12 + c12_a, "          |")
    print(colored("--" * 70, 'green'))
